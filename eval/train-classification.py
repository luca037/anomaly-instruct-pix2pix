...
Differences vs. upstream:
  ...
  * Training data is read from the MVTec-mirrored layout written by
    ``eval/generate_defects.py``  (``<generated_path>/<object>/test/<defect>/``).
...
from common import DEFAULT_CLASSIFICATION_CKPT, add_common_args, add_training_args, plot_losses
...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        criterion = nn.CrossEntropyLoss()
        dataloader = DataLoader(dataset, batch_size=args.bs, shuffle=True, num_workers=args.num_workers)
        ...
        plot_losses(train_losses, val_losses, run_name, args.checkpoint_path)
