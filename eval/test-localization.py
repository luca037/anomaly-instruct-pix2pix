...
from common import DEFAULT_LOCALIZATION_CKPT, add_common_args
from unet_utils.au_pro_util import calculate_aupro
from unet_utils.data_loader import AnomalyLocalizationTestDataset


def parse_args():
    parser = argparse.ArgumentParser(description="Test per-object anomaly localization segmentors.")
    add_common_args(parser, ckpt_default=DEFAULT_LOCALIZATION_CKPT)
...
def _eval_defect_names(args, obj_name):
    ...
    gen_test = os.path.join(args.generated_path, obj_name, "test")

    mvtec_defects = (
        {d for d in os.listdir(mvtec_test) if os.path.isdir(os.path.join(mvtec_test, d)) and d != "good"}
...
def _metrics_for(anomaly_name, gts, preds):
    ...
    au_pro, _, _ = calculate_aupro([g.astype(np.float32) for g in gts], [p.astype(np.float32) for p in preds])
    return {
        "defect": anomaly_name,
        "n": len(gts),
        "pixel_AUROC": p_auroc,
...
def evaluate_object(args, obj_name, anomaly_names, all_test_names, ckpt_path):
    ...
    grouped_gts, grouped_preds = predict_object(args, obj_name, all_test_names, ckpt_path)
    rows = []
    for defect in anomaly_names:
        ...
        rows.append(m)
        print(f"    {defect:12s} n={m['n']:>3d} pxAUROC={m['pixel_AUROC']:.3f} pxAP={m['pixel_AP']:.3f} pxF1={m['pixel_F1']:.3f} pxPRO={m['pixel_AUPRO']:.3f}")
    if rows:
        ...
        print(f"    [pixel-avg] AUROC={avg_px_auroc:.3f} AP={avg_px_ap:.3f} F1={avg_px_f1:.3f} PRO={avg_px_pro:.3f}")
    i_auroc, i_ap, i_f1 = _image_level_metrics(grouped_gts, grouped_preds)
    print(f"    [image-level] AUROC={i_auroc:.3f} AP={i_ap:.3f} F1={i_f1:.3f}")
...
def main():
    ...
    for obj_name in args.categories:
        ...
        if not anomaly_names:
            ...
            continue
        rows, image_metrics = evaluate_object(args, obj_name, anomaly_names, all_test_names, ckpt)
...
def _write_csv(args, all_rows, image_summary):
    ...
    fields = ["object", "defect", "n", "image_AUROC", "image_AP", "image_F1", "pixel_AUROC", "pixel_AP", "pixel_F1", "pixel_AUPRO"]
    with open(csv_path, "w", newline="") as f:
        ...
        for obj_name, (auroc, ap, f1) in image_summary:
            w.writerow({"object": obj_name, "defect": "image-level", "image_AUROC": auroc, "image_AP": ap, "image_F1": f1})
    print(f"  wrote {csv_path}")


def _print_summary(all_rows, image_summary):
    metrics = ["pixel_AUROC", "pixel_AP", "pixel_F1", "pixel_AUPRO"]
    for metric in metrics:
        vals = [r[metric] for _, r in all_rows if isinstance(r.get(metric), float)]
