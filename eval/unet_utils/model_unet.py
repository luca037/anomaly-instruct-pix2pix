"""
Anomaly-detection segmentation networks, ported from O2MAG:
`eval/unet_utils/model_unet.py`.

Two models are defined upstream:

* ``ReconstructiveSubNetwork``  — for the reconstruction branch.
* ``DiscriminativeSubNetwork``  — for the segmentation/segmentation branch
  (the one DRAEM trains on synthetic anomalies + good normals).

Construction is device-safe: this module defines no device handling — callers
create the model and call ``.to(args.device)`` themselves. The only change from
upstream is appending a final ``nn.Sigmoid()`` to ``DiscriminativeSubNetwork``'s
output (DRAEM's ``SegDecNet`` sigmoid is already implicit via the seg output; the
localization trainer below expects probabilities).
"""
