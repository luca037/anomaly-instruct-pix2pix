...
Training data:
  * synthetic defects + their **stored** on-disk Otsu masks from
    ``<generated_path>/<object>/test/<defect>`` + ``ground_truth/<defect>``
...
from common import DEFAULT_LOCALIZATION_CKPT, add_common_args, add_training_args, plot_losses
from unet_utils.data_loader import AnomalyLocalizationTrainDataset, AnomalyLocalizationTestDataset
from unet_utils.loss import FocalLoss, SSIMLoss
from unet_utils.model_unet import DiscriminativeSubNetwork


def parse_args():
    parser = argparse.ArgumentParser(description="Train per-object anomaly-detection segmentors.")
...
def _object_defect_types(args, obj_name):
    ...
    test_root = os.path.join(args.generated_path, obj_name, "test")
    if os.path.isdir(test_root):
        return [d for d in sorted(os.listdir(test_root)) if os.path.isdir(os.path.join(test_root, d))]
...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [int(args.epochs * 0.8), int(args.epochs * 0.9)], gamma=0.2)
        ...
        plot_losses(train_losses, val_losses, run_name, args.checkpoint_path, ylabel="focal loss")
